...
class ModelArguments:
    # LLM Arguments
    model_type: Optional[str] = field(default="videollama3", metadata={"help": "Model type"})
    model_path: Optional[str] = field(default="lmsys/vicuna-7b-v1.5")
    version: Optional[str] = field(default="v1", metadata={"help": "Version of the conversation template."})
    freeze_backbone: bool = field(default=False, metadata={"help": "Whether to freeze the LLM backbone."})
    # Connector Arguments
    vision_projector_type: Optional[str] = field(default='mlp2x_gelu')
...
class DataArguments:
    ...
    max_frames: Optional[int_or_none] = field(default=None)
    ...
    data_lengths_path: Optional[str] = field(default=None, metadata={"help": "Path to the data lengths."})
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str, vlprocessor, data_args: DataArguments):
        ...
        try:
            for data in data_path:
                features=Features({'id': Value('string'), 'image': ds_List(Value('string')), 'video': ds_List(Value('string')), 'conversations': ds_List({'from': Value('string'), 'value': Value('string')}), 'width': ds_List(Value('int64')), 'height': ds_List(Value('int64')), 'duration': ds_List(Value('float64'))})
                ...
                if data.endswith(".json") or data.endswith(".jsonl"):
                    ...
                    data_objs.append(load_dataset("json", data_files=data, cache_dir=data_args.dataset_cache_dir, features=features)["train"])
                else:
        ...
        if data_args.data_lengths_path is not None:
            self.lengths = torch.load(data_args.data_lengths_path)
        else:
            self.lengths = None
    ...
    def _convert_normal(self, data_dict):
        ...
        if 'image' in data_dict and data_dict['image'] is not None and len(data_dict['image']) > 0:
            ...
            frame_types = [0] * len(images)
        elif 'video' in data_dict and data_dict['video'] is not None and len(data_dict['video']) > 0:
            modal = 'video'
            if all(not DEFAULT_VIDEO_TOKEN in sentence["value"] for sentence in conversation):
                warnings.warn(f"Video tag not found in the conversation, add it automatically at the beginning!")
                conversation[0]["value"] = f"{DEFAULT_VIDEO_TOKEN}" + conversation[0]["value"]
            ...
            if isinstance(video_file, list) and len(video_file) == 1:
                ...
                images, timestamps, frame_types = load_video(video_file, fps=self.data_args.fps, max_frames=self.data_args.max_frames)
            elif len(video_file) > 1:
                raise NotImplementedError("Multiple video files are not supported yet!")
            else:
        ...
        for conv in conversation:
            if conv["from"] == "human":
                if modal == "image":
                    chunks = conv["value"].split(DEFAULT_IMAGE_TOKEN)
                elif modal == "video":
                    chunks = conv["value"].split(DEFAULT_VIDEO_TOKEN)
                elif modal == "text":
                    chunks = [conv["value"]]
                else:
                ...
                for chunk_idx in range(1, 2 * len(chunks)):
                    if chunk_idx % 2 == 1:
                        ...
                        if modal == 'image':
                            ...
                            messages[-1]["content"].append({"type": "video", "num_frames": len(images), "timestamps": timestamps})
            else:
                messages.append({
                    "role": "assistant",
                    "content": conv['value']
        ...
        if modal == 'image' or modal == 'video':
            modal = [modal] * len(images)

        return modal, images, messages, merge_size, frame_types

    def load_data(self, i):
        data_dict = self.list_data_dict[i]

        modal, images, messages, merge_size, frame_types = self._convert_normal(data_dict)
        ...
        elif modal[0] == 'image' or modal[0] == 'video':
            assert len(data_dict['pixel_values']) > 0 and len(data_dict['grid_sizes']) > 0, f"Invalid image data: {data_dict['images']}, {data_dict['grid_sizes']}"
...
def make_supervised_data_module(vlprocessor, data_args) -> Dict:
    ...
    assert not data_args.use_dynamic_batching
...
class DataCollatorWithFlatteningForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict], separator_id=-100) -> Dict[str, torch.Tensor]:
        ...
        for idx in range(0, len(input_ids)):
            ...
            if "position_ids" in instances[idx]:
                position_ids.append(instances[idx]["position_ids"][:self.vlprocessor.tokenizer.model_max_length])
            else:
        ...
        if "input_features" in instances[0]:
            batch["audio_input_features"] = torch.cat([x["input_features"] for x in instances])
            batch["audio_attention_mask"] = torch.cat([x["attention_mask"] for x in instances])
        batch["modals"] = sum([x["modals"] for x in instances], [])
        batch["indices"] = [x["index"] for x in instances]
...
def train(attn_implementation=None):
    ...
    config = CONFIG_MAPPING[model_args.model_type].from_pretrained(model_args.model_path)
    ...
    if model_args.vision_encoder is not None:
        ...
        config.vision_projector_type = model_args.vision_projector_type
    model = MODEL_FOR_CAUSAL_LM_MAPPING[type(config)].from_pretrained(
    ...
    if model_args.vision_encoder is not None:
        ...
        vision_projector = model.get_vision_projector()
        vision_projector.to(dtype=compute_dtype if training_args.bf16 else torch.float16, device=training_args.device)
        ...
        if training_args.bits in [4, 8]:
            model.get_model().vision_projector.to(dtype=compute_dtype, device=training_args.device)
    ...
    if model_args.vision_encoder is not None:
        ...
        model.config.vision_projector_lr = training_args.vision_projector_lr
        model.config.embedding_lr = training_args.embedding_lr
        ...
        if model.config.llm_lr is None:
            ...
            if model.config.use_reconstruct:
                for n, p in model.get_model().named_parameters():
                    if "vision_distill_layer" in n:
                        p.requires_grad = True
        ...
        if model.config.vision_projector_lr is None:
            for p in model.get_model().vision_projector.parameters():
                p.requires_grad = False
        ...
        model.config.image_token_length = data_args.image_token_length = vision_projector.cal_proj_size(vision_encoder.num_patches_per_side)
        # 3. set spatial merge size as default
        tokenizer.add_tokens([DEFAULT_IMAGE_TOKEN], special_tokens=True)
        model.config.image_token_index = tokenizer.convert_tokens_to_ids(DEFAULT_IMAGE_TOKEN)

        vlprocessor = PROCESSOR_MAPPING[type(config)](vision_encoder.image_processor, tokenizer)
    ...
    # select a Trainer
    trainer = PenguinVLTrainer(model=model, processing_class=vlprocessor, args=training_args, **data_module)
