...
# Adapted from 2024 Alibaba DAMO Academy
#
...
class IdentityMap(nn.Module):
    ...
    @property
    def config(self):
        return {"vision_projector_type": 'identity'}
...
def build_vision_projector(config, vision_hidden_size, delay_load=False, **kwargs):
    projector_type = getattr(config, 'vision_projector_type', 'mlp2x_gelu')
    hidden_size = config.hidden_size

    if projector_type == "linear":
        return nn.Linear(vision_hidden_size, hidden_size)
    elif  projector_type == "simp_spatial_conv":
        return SimSpatialConv(vision_hidden_size, hidden_size)
    elif projector_type.startswith("mlp"):
        return MlpGeluProjector(vision_hidden_size, hidden_size, projector_type)
